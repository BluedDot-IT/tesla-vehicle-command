...
func (v *Vehicle) AddKey(ctx context.Context, publicKey *ecdh.PublicKey, isOwner bool, formFactor vcsec.KeyFormFactor) error {
	if isOwner {
		return v.AddKeyWithRole(ctx, publicKey, keys.Role_ROLE_OWNER, formFactor, nil, 0)
	}
	return v.AddKeyWithRole(ctx, publicKey, keys.Role_ROLE_DRIVER, formFactor, nil, 0)
}
...
func (v *Vehicle) AddKeyWithRole(ctx context.Context, publicKey *ecdh.PublicKey, role keys.Role, formFactor vcsec.KeyFormFactor, permissions []vcsec.WhitelistKeyPermission_E, secondsActive uint32) error {
	if publicKey.Curve() != ecdh.P256() {
		return protocol.ErrInvalidPublicKey
	}
	payload := addKeyPayload(publicKey, role, formFactor, permissions, secondsActive)
...
func (v *Vehicle) RemoveAllImpermanentKeys(ctx context.Context) error {
	payload := vcsec.UnsignedMessage{
		SubMessage: &vcsec.UnsignedMessage_WhitelistOperation{
			WhitelistOperation: &vcsec.WhitelistOperation{
				SubMessage: &vcsec.WhitelistOperation_RemoveAllImpermanentKeys{
...
func (v *Vehicle) SendAddKeyRequest(ctx context.Context, publicKey *ecdh.PublicKey, isOwner bool, formFactor vcsec.KeyFormFactor) error {
	if isOwner {
		return v.SendAddKeyRequestWithRole(ctx, publicKey, keys.Role_ROLE_OWNER, formFactor, nil, 0)
	}
	return v.SendAddKeyRequestWithRole(ctx, publicKey, keys.Role_ROLE_DRIVER, formFactor, nil, 0)
}
...
//
// [Protocol Specification]: https://github.com/teslamotors/vehicle-command/blob/main/pkg/protocol/protocol.md#roles
func (v *Vehicle) SendAddKeyRequestWithRole(ctx context.Context, publicKey *ecdh.PublicKey, role keys.Role, formFactor vcsec.KeyFormFactor, permissions []vcsec.WhitelistKeyPermission_E, secondsActive uint32) error {
	if publicKey.Curve() != ecdh.P256() {
		return protocol.ErrInvalidPublicKey
	}
	if _, ok := v.conn.(connector.FleetAPIConnector); ok {
	...
	encodedPayload, err := proto.Marshal(addKeyPayload(publicKey, role, formFactor, permissions, secondsActive))
	if err != nil {
		return err
	}
